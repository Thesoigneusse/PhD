...
class Doc2DocTranslation(TranslationTask):
    ...
    def add_args(parser):
        ...
        parser.add_argument('--mode',type=str,default=None,help='one of: none, block, n2n_block, slide_n2one, slide_n2n, slide_block, complete')
        parser.add_argument('--num-tok',type=int,metavar='N',default=1000,help='max length of documents in number of tokens')
        parser.add_argument('--num-sent',default=None,type=int,metavar='N',help='max length of documents in number of sentences')
        parser.add_argument('--add-doc-markup',action='store_true',help='add markup to documents i.e. special symbols <BEG>, <BRK>, <CNT>, <END>')
        parser.add_argument('--remove-markup-from-output',action='store_true',help='remove special symbols from generator output') # to use only at generation if --add-doc-markup was used    
        parser.add_argument('--need-seg-label',default='False', type=str, metavar='BOOL',help='Compute segment labels for current segments (label=1) and context (label=2,3,...).')
        parser.add_argument('--context-discount',default=1,type=float,metavar='D',help='discount weight to apply to the loss generated by the context sentences, 1 means no discount')
        parser.add_argument('--original-loss-for-stopping', choices=["True", "False"], default="True",type=str, metavar='BOOL' , help='Use the non-discounted loss as a stopping criterion even if context loss is being discounted.') # action='store_true'
        parser.add_argument('--need-encoder-self-attn',type=int,default=0,help='if >1, retrieve encoder attention weights to analyze them')
        parser.add_argument('--need-decoder-self-attn',type=int,default=0,help='if >1, retrieve decoder attention weights to analyze them')
        ...
        with torch.autograd.profiler.record_function("forward"):
            if self.context_discount == 1:
                ...
                if self.original_loss_for_stopping:
                    ...
                    # print(f"[DEBUG] loss.data: {loss.data}")
                    # print(f"[DEBUG] weighted_loss: {logging_output['full_loss']}")
