...
def softmax_1(x, dim=-1):
    z = x - torch.max(x, dim=dim)[0].unsqueeze(dim=dim) # "t" was replace by "x" compared to Marco's code
    num = torch.exp(z)
    den = torch.sum(num, dim=dim)
    return torch.div( num, 1+den.unsqueeze(dim=dim) )

@with_incremental_state
class QuietMultiheadAttention(nn.Module):
    """Quiet Multi-headed attention.
    Use a modified softmax to allow for no attention decisions.
    ...
    def forward(
        ...
        if saved_state is not None:
            ...
            key_padding_mask = QuietMultiheadAttention._append_prev_key_padding_mask(
        ...
        attn_weights = QuietMultiheadAttention.apply_sparse_mask(attn_weights, tgt_len, src_len, bsz)
        ...
        #)
        attn_weights_float = softmax_1(attn_weights, dim=-1)
