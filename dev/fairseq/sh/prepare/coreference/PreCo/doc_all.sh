#!/usr/bin/env bash
## TODO 
# À reprendre lorsque PreCo sera dispo


src=en
DATA=$HOME/PhD/dev/fairseq/data/coreference
# orig=/video/getalp/
orig=$HOME/temp/orig/coreference/ # à remplir
...
if [ $1 = "standard" ]; then
    ...
    echo "Pre-processing data ..."
    for i in "input" "output"; do
        cp $orig/newlist.txt.$i $tmp/$i.tok.$src
    done
    ...
    for i in "input" "output"; do
        ...
        mv $tmp/$i.tok.$src $prep/$i.tok.$src
        mv $tmp/$i.tok.$src.heads $prep/$i.tok.$src.heads
    done

    echo "Printing stats train stats..."
    for l in "input" "output"; do
        echo "Stats for train.$src (tokenized and cleaned):"
        words=$(wc -w $prep/$l.tok.$src | awk '{print $1;}')
        sents=$(wc -l $prep/$l.tok.$src | awk '{print $1;}')
        printf "%10d words \n" $words
